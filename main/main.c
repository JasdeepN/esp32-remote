...
static void set_led(int i, double s);
void startup(void);
...
static const char *CONFIG_SNTP_TIME_SERVER = "time.nrc.ca";

static const double INTENSITY = 0.10;
...
static void set_led(int i, double intensity)
{
   ESP_LOGI("Set LED", "Set LED to (%f,%f,%f)", ceil(status[i].red*intensity), ceil(status[i].green*intensity), ceil(status[i].blue*intensity));
   
   led_strip_set_pixel(led_strip, 0, ceil(status[i].red*intensity), ceil(status[i].green*intensity), ceil(status[i].blue*intensity));
   /* Refresh the strip to send data */
   led_strip_refresh(led_strip);
...
static char* get_time(){
   ...
   static char payload[100]; // can shrink this 
   sprintf(payload, "Current Time : %s", time_str);
